...
<div class="container-xxl flex-grow-1 container-p-y">
    <div class="container-fluid p-0">
        ...
        {{-- Pencarian --}}
        <div class="mb-4">
            <input type="text" id="searchVehicle" class="form-control form-control-sm" placeholder="Cari kendaraan berdasarkan merek atau plat..." style="max-width: 350px;">
        </div>

        {{-- Daftar Kendaraan --}}
        <div class="row" id="vehicleList">
            @forelse ($vehicles as $vehicle)
                <div class="col-md-4 mb-4 vehicle-card" data-name="{{ strtolower($vehicle->brand . ' ' . $vehicle->license_plate) }}">
                    <div class="card border-0 shadow-sm h-100 rounded-4 overflow-hidden">
                        ...
                        <div class="card-body d-flex flex-column">
                            ...
                            <ul class="list-unstyled small mb-3">
                                <li><i class="bi bi-palette me-2 text-muted"></i> Warna: {{ ucfirst($vehicle->color) ?? '-' }}</li>
                                <li><i class="bi bi-gear-wide-connected me-2 text-muted"></i> Transmisi: {{ $vehicle->transmission_indo }}</li>
                                <li><i class="bi bi-fuel-pump me-2 text-muted"></i> BBM: {{ $vehicle->fuel_type_indo }}</li>
                                <li><i class="bi bi-speedometer2 me-2 text-muted"></i> CC: {{ $vehicle->engine_capacity ?? '-' }}</li>
                            </ul>

                            <div class="mt-auto d-flex justify-content-between">
                                <a href="{{ route('vehicles.show', $vehicle->id) }}" class="btn btn-outline-primary btn-sm w-50 me-2">
                                    <i class="bi bi-eye"></i> Lihat
                                </a>
                                <a href="{{ route('vehicles.edit', $vehicle->id) }}" class="btn btn-outline-warning btn-sm w-50">
